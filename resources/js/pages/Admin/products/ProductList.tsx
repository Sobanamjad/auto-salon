...
export default function ProductList({ products = [], title = '會員商品' }: Props) {
    ...
    const handleSortUpdate = (id: number, sortOrder: number) => {
        router.put(`/admin/products/${id}/sort`, { sort_order: sortOrder });
    };

    const handleResetViews = (id: number, name: string) => {
        if (confirm(`確定要清除: ${name} 點閱人紀錄嗎？`)) {
            router.get(`/admin/products/${id}/reset-views`);
        }
    };

    const handleToggleHome = (id: number) => {
        router.get(`/admin/products/${id}/toggle-home`);
    };
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 text-gray-900">
                ...
                <div className="border-b border-gray-200 pb-4 mb-6">
                    <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                        <FaGift className="text-blue-500" /> {title}
                    </h2>
                    <p className="text-sm text-gray-500 mt-1">管理所有會員商品</p>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 border">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase w-20">推廣行銷</th>
                                <th className="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase w-20">複製</th>
                                ...
                                <tr>
                                    <td colSpan={9} className="px-3 py-8 text-center text-gray-500">
                                        <div className="flex flex-col items-center gap-2">
                                            <FaGift size={32} className="text-gray-300" />
                                            <p>暫無商品資料</p>
                                            <Link
                                                href="/admin/products/create"
                                ...
                                currentItems.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-3 py-2">
                                            ...
                                            {item.product_no && (
                                                <div className="text-xs text-gray-400">#{item.product_no}</div>
                                            )}
                                        </td>
                                        <td className="px-3 py-2 text-center">
                                            <span className="font-bold">{item.views || 0}</span>
                                            <button
                                                onClick={() => handleResetViews(item.id, item.name)}
                                        ...
                                        <td className="px-3 py-2 text-center">
                                            <Link
                                                href={`/admin/products/${item.id}/promote`}
                                        ...
                                        <td className="px-3 py-2 text-center">
                                            <Link
                                                href={`/admin/products/${item.id}/copy`}
                                            ...
                                            <div className="border-t border-dashed border-gray-300 my-1"></div>
                                            <Link
                                                href={`/admin/products/${item.id}/preview`}
                                                target="_blank"
