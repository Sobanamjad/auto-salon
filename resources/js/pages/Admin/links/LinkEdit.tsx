...
export default function LinkEdit({ link, title }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        language: link.language || 'TS',
        status: link.status ?? true,
        show_on_home: link.show_on_home ?? false,
        show_on_sidebar: link.show_on_sidebar ?? true,
        sort_order: link.sort_order || 999,
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                                <FaLink className="text-blue-500" /> {title}
                            </h2>
                            <p className="text-sm text-gray-500 mt-1">編輯連結 #{link.id}</p>
                        </div>
                    </div>
