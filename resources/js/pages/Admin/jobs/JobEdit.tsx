...
export default function JobEdit({ job, title }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        language: job.language || 'TS',
        status: job.status ?? true,
        show_on_home: job.show_on_home ?? false,
        sort_order: job.sort_order || 999,
        published_start: job.published_start || new Date().toISOString().split('T')[0],
        published_end: job.published_end || '2200-12-31',
        job_no: job.job_no || '',
        company: job.company || '',
        contact_person: job.contact_person || '',
        contact_gender: job.contact_gender || '先生',
        contact_phone: job.contact_phone || '',
        contact_mobile: job.contact_mobile || '',
        contact_email: job.contact_email || '',
        contact_web: job.contact_web || '',
        work_location: job.work_location || '',
        work_area: job.work_area || '台南市仁德區',
        nearby_school_1: job.nearby_school_1 || '',
        nearby_school_2: job.nearby_school_2 || '',
                ...
                <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">編輯職缺 #{job.id}</p>
                        </div>
                    </div>
